...
export const getTransactionsTool = {
    ...
    execute: async (args) => {
        try {
            const client = createApmClient(args);
            const transactions = await client.getTransactions(args.serviceName, args.startTime, args.endTime, args.transactionType, args.limit);
            return JSON.stringify({
...
export const getErrorsTool = {
    ...
    execute: async (args) => {
        try {
            const client = createApmClient(args);
            const errors = await client.getErrors(args.serviceName, args.startTime, args.endTime, args.limit);
            return JSON.stringify({
                serviceName: args.serviceName,
                errors,
                count: errors.length,
...
export const getLatencyStatsTool = {
    ...
    execute: async (args) => {
        try {
            const client = createApmClient(args);
            const stats = await client.getLatencyStats(args.serviceName, args.startTime, args.endTime, args.transactionType);
            return JSON.stringify({
                serviceName: args.serviceName,
                latencyMs: {
...
export const getErrorRateTool = {
    ...
    execute: async (args) => {
        try {
            const client = createApmClient(args);
            const stats = await client.getErrorRate(args.serviceName, args.startTime, args.endTime);
            return JSON.stringify({
                serviceName: args.serviceName,
